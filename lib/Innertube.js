...
app.listen(6746, () => {
  console.log(`Server running in 6746`);
});
// Simple get method, all type of methods can be put here
app.get("/buscar/:busqueda", async function (req, res) {
  const youtube = await new Innertube({ gl: "US" });
  let resp = await youtube.search(req.params.busqueda);
  res.json({ url: resp.videos[0].url });
});

// END OF Deagle50's additions
...
class Innertube {
  ...
  async #setSetting(setting_id, type, new_value) {
    ...
    if (!values.hasOwnProperty(new_value)) throw new Utils.InnertubeError("Invalid option", { option: new_value, available_options: Object.keys(values) });
  ...
  async #getBasicChannelAnalytics() {
    ...
    const action = await this.actions.browse("FEanalytics_screen", { params, is_yta: true });
    return action;
  }
  ...
  async getDetails(video_id) {
    ...
    const livechat_ctoken = continuation.data.contents?.twoColumnWatchNextResults?.conversationBar?.liveChatRenderer?.continuations?.find(
      (continuation) => continuation.reloadContinuationData
    ...
    details.setNotificationPreferences = (type) =>
      this.actions.notifications("modify_channel_preference", { channel_id: details.metadata.channel_id, pref: type || "NONE" });
  ...
  #chooseFormat(options, video_data) {
    ...
    !streams || (!streams.length && (streams = filtered_formats.filter((format) => format.quality == "medium")));
  ...
  async getStreamingData(video_id, options = {}) {
    ...
    if (!streaming_data.selected_format) throw new Utils.NoStreamingDataError("Could not find any suitable format.", { video_id, options });
    ...
    this.actions.getVideoInfo(video_id, cpn).then(async (video_data) => {
      if (video_data.playabilityStatus.status === "LOGIN_REQUIRED")
        return stream.emit("error", {
          message: "You must login to download age-restricted videos.",
          error_type: "LOGIN_REQUIRED",
          playability_status: video_data.playabilityStatus.status,
        });
      ...
      if (!format) return stream.emit("error", { message: "Could not find any suitable format.", type: "FORMAT_UNAVAILABLE" });
